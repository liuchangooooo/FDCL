...
class AntMazeEnv(MazeEnv, EzPickle):
    ...
    * v2 & v1: legacy versions in the [D4RL](https://github.com/Farama-Foundation/D4RL).
    ...
    def __init__(
        ...
        goal_dist_threshold: Optional[float] = None,
        **kwargs,
    ):
    ...
    def step(self, action):
        ...
        reward, reward_dict = self.compute_reward_curriculum(obs["achieved_goal"], info)
    ...
    def compute_reward_curriculum(self, achieved_goal: Optional[np.ndarray] = None, info: Optional[Dict] = None):
        if achieved_goal is None:
            achieved_goal = self.torso_coordinate(self.get_ant_obs())
        if info is None:
            info = {}

        goal_distance = float(np.linalg.norm(achieved_goal[:2] - self.goal))
        reward = self.compute_reward(achieved_goal[:2], self.goal, info)
        reward_scalar = float(np.asarray(reward).squeeze())
        reward_dict = {
            "goal_reward": reward_scalar,
            "goal_distance": goal_distance,
        }

        return reward_scalar, reward_dict
